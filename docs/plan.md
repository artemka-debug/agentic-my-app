## Local-Only Cursor SDK agentic-my-app Spec

### Core Position
Build **agentic-my-app** as a TypeScript CLI/service using `@cursor/sdk` with **local runtime only**. In this spec, `localOnly` means Cursor agents run through the SDK local runtime against a checked-out local workspace/worktree, not Cursor Cloud.
...
1. **CLI Entrypoint**
   - `agentic-my-app issue <owner/repo#number>`
   - `agentic-my-app task <task-file>`
   - `agentic-my-app resume <run-id>`
   - `agentic-my-app monitor-pr <pr-url>`
   - `agentic-my-app status <run-id>`
...
3. **State Store**
   - Local filesystem state under `.agentic-my-app/runs/<runId>/`.
...
8. The CLI creates parallel worktrees.
9. Test writer agents create focused tests or explicit test expectations before implementation.
10. Implementation agents work independently against those tests/expectations.
11. Verifier runs checks and reviews each candidate.
12. Failed candidates receive structured feedback and retry.
13. Best passing candidate is selected.
14. Review agents produce code, security, and PO acceptance recommendations.
15. Cleanup agent applies verifier/review recommendations and reruns required checks.
16. Finalization agent polishes, squashes or organizes commits, updates docs/tests, prepares PR.
17. Local git pushes branch.
18. GitHub PR is created.
19. PR monitor watches CI and comments.
20. Fix agents address PR feedback until done or blocked.
...
The CLI runs from a canonical local repo checkout.
...
```text
agentic-my-app/<issue-number>/<runId>/impl-01
agentic-my-app/<issue-number>/<runId>/impl-02
agentic-my-app/<issue-number>/<runId>/final
agentic-my-app/<issue-number>/<runId>/fix-01
```
...
- No shared mutable files except run artifacts under `artifacts.root`.
...
```bash
agentic-my-app issue owner/repo#123
agentic-my-app issue https://github.com/owner/repo/issues/123
agentic-my-app issue 123 --repo owner/repo
```
...
Rules:
- Do not create a PR.
- Do not modify run artifacts created under `artifacts.root`.
...
FR-002: The CLI creates isolated worktrees for parallel implementers.
...
The CLI should infer test commands from:
...
gh pr create \
  --repo owner/repo \
  --title "<title>" \
  --body-file .agentic-my-app/runs/<runId>/artifacts/pr-body.md
```
...
- Polling interval.
- Manual `agentic-my-app monitor-pr`.
...
Example `agentic-my-app.config.yaml`:
...
agentic-my-app issue owner/repo#123
agentic-my-app issue https://github.com/owner/repo/issues/123
agentic-my-app task ./task.md
agentic-my-app approve <runId> spec
agentic-my-app approve <runId> decomposition
agentic-my-app run <runId>
agentic-my-app resume <runId>
agentic-my-app status <runId>
agentic-my-app candidates <runId>
agentic-my-app verify <runId> --candidate impl-01
agentic-my-app finalize <runId> --candidate impl-01
agentic-my-app create-pr <runId>
agentic-my-app monitor-pr <pr-url>
agentic-my-app pause <runId>
agentic-my-app cancel <runId>
agentic-my-app cleanup <runId>
```

Autonomous mode:

```bash
agentic-my-app issue owner/repo#123 --autonomous
agentic-my-app issue owner/repo#123 --no-spec-gate --no-decomposition-gate --auto-pr
```
